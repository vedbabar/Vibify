...
const AISidebar = ({
	...
	const handleSearch = async () => {
		...
		try {
			...
			setVideo(res.data); // assumes backend returns { videoId, title, thumbnail, channel }
		} catch (err) {
				  ...
				  <div className="glass-morphism rounded-3xl overflow-hidden hover:scale-[1.02] transition-all duration-500 group shadow-2xl">
					...
					<div className="relative overflow-hidden">
					  ...
					  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center">
						<div className="transform scale-75 group-hover:scale-100 transition-transform duration-300">
						  <button className="w-16 h-16 bg-white text-slate-900 rounded-full flex items-center justify-center hover:bg-slate-100 transition-colors duration-200 shadow-2xl hover:shadow-white/25">
					...
					<div className="p-6 space-y-4">
					  ...
					  <div className="flex gap-3 pt-2">
						<button className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-lg hover:shadow-emerald-500/25">
						  <Play className="w-4 h-4" fill="currentColor" />
						  <span>Play</span>
						</button>
						<button className="w-12 h-12 glass-morphism hover:bg-slate-700/50 rounded-xl flex items-center justify-center transition-all duration-200 group hover:scale-110">
						  <Heart className="w-4 h-4 text-slate-400 group-hover:text-red-400 group-hover:fill-current transition-all duration-200" />
						</button>
						<button className="w-12 h-12 glass-morphism hover:bg-slate-700/50 rounded-xl flex items-center justify-center transition-all duration-200 group hover:scale-110">
						  <ExternalLink className="w-4 h-4 text-slate-400 group-hover:text-indigo-400 transition-colors duration-200" />
						</button>
					  </div>
					</div>
				  </div>
